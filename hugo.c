...
#define HUGO_DICTTABLE_OFFSET 0x15
...
static int32 get_story_file_IFID(void *s_file, int32 extent, char *output, int32 output_extent)
{
 ...
 long dicttable =
  read_hugo_addx((unsigned char *)story_file + HUGO_DICTTABLE_OFFSET) * 16;
 ...
 for(i = 0; i < extent - 28; i++)
 {
      ...
      story_file[i + 15] == hyphen)
  {
   for (j = 0; j < 36; j++)
   {
    /* One Hugo game, PAXLess, uses lowercase letters.
     * We change it to uppercase here
     */
 ...
 */
 ...
 if (sf[0] < 34)
  ...
  scale = 16;
 for(i = 3; i < 0x0B; i++)
  if (sf[i] < 0x20 || sf[i] > 0x7e)
   return INVALID_STORY_FILE_RV;
 for(i = 0x0b; i < 0x18; i += 2)
  if (read_hugo_addx(sf + i) * scale > extent)
   return INVALID_STORY_FILE_RV;
